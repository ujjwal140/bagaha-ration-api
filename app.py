...
def home():
    ...
    <html>
        <head>
            <title>Bagaha Seth ERP v2</title>
            ...
            <style>
                * { box-sizing: border-box; }
                ...
                .add-btn { background: #27ae60; color: white; border: none; padding: 15px; width: 100%; border-radius: 8px; font-size: 18px; font-weight: bold; cursor: pointer; }
                .table-wrapper { overflow-x: auto; margin-top: 20px; }
                table { width: 100%; border-collapse: collapse; min-width: 400px; }
                th, td { padding: 10px; text-align: left; border-bottom: 1px solid #eee; font-size: 14px; }
                th { background: #f8f9fa; color: #7f8c8d; }
                .unit-tag { background: #e8f5e9; color: #2e7d32; padding: 2px 6px; border-radius: 4px; font-weight: bold; }
            </style>
        </head>
        <body>
            <div class="container">
                ...
                <div style="background: #fdfdfd; padding: 15px; border-radius: 10px; border: 1px dashed #ccc;">
                    <div class="form-group"><label>Samaan</label><input type="text" id="name" placeholder="Aalu"></div>
                    ...
                    <button class="add-btn" onclick="addItem()">➕ Save Karein</button>
                </div>
                ...
                async function loadItems() {
                    ...
                    data.forEach(item => {
                        body.innerHTML += `<tr>
                            ...
                            <td><span class="unit-tag">${item.total_base_weight} ${item.unit || 'kg'}</span></td>
                        </tr>`;
                    });
                }
